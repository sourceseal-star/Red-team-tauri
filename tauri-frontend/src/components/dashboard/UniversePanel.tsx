import { useCallback, useEffect, useState } from 'react'
    ...
    type UniversePayload = { module?: string; state?: Record<string, unknown>; network_context?: Record<string, unknown>; observed_at?: string }
    const ACTIONS = [
    ...
    return (
      <div className="min-h-full bg-slate-950 text-slate-100 p-4 md:p-6 space-y-5">
        ...
        <section className="rounded-xl border border-slate-800 bg-slate-900/70 p-4"><h2 className="font-medium mb-3">Contexto del dispositivo</h2><div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm"><div><span className="text-slate-500">Host:</span> {String(network.hostname || '—')}</div><div><span className="text-slate-500">IP local:</span> {String(network.local_ip || '—')}</div><div><span className="text-slate-500">Stack:</span> {String(network.stack || '—')}</div><div><span className="text-slate-500">Privilegios:</span> {String(network.privilege || '—')}</div></div><p className="text-xs text-slate-500 mt-4">Última sincronización: {String(state.last_synchronization || 'todavía no')}</p><p className="text-xs text-slate-600 mt-1">Estado observado: {String(status?.observed_at || '—')}</p></section>
      </div>
    )
    }
