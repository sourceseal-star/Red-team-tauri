...
import { Wifi, Camera, Send, Mic, Radio, Sliders, AlertTriangle, Activity } from 'lucide-react';
...
export default function WarRoom() {
    ...
    try {
      ...
      if (data.motion_detected) {
        setMotionStatus(prev => ({
          ...prev, [cam.ip]: `🚨 ${data.captures?.length || 0} capturas con hash`
        }));
      } else {
  ...
  return (
    <div className="h-full w-full flex flex-col gap-3 font-mono">
        ...
        <div className="bg-[var(--ss-bg-2)] border border-[var(--ss-border)] rounded-lg overflow-hidden flex flex-col">
          <div className="p-2 bg-[var(--ss-bg-3)] border-b border-[var(--ss-border)] flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <span className="text-[9px] text-gray-500">{hosts.length} hosts</span>
              <button
                onClick={runTopology}
                disabled={loading}
          ...
          {/* Hosts seleccionables para traceroute */}
          {hosts.length > 0 && (
      ...
      {/* FILA INFERIOR: Comunicaciones Ultrasónicas */}
      <div className="h-56 bg-[var(--ss-bg-2)] border border-[var(--ss-border)] rounded-lg overflow-hidden flex flex-col shrink-0">
        <div className="p-2 bg-[var(--ss-bg-3)] border-b border-[var(--ss-border)] flex items-center justify-between">
          <span className="text-xs text-cyan-400 flex items-center gap-2">
        ...
        <div className="flex-1 flex min-h-0">
          ...
          <div className="flex-1 p-3 flex flex-col gap-2">
            ...
            <div className="flex items-center gap-3 text-[9px] text-gray-500">
              <span>Freq base: <span className="text-cyan-400">{(18000 + freqOffset).toLocaleString()} Hz</span></span>
              <span>·</span>
              <span>Sync: <span className="text-amber-400">{(19500 + freqOffset).toLocaleString()} Hz</span></span>
              <span>·</span>
              <span>Modo: <span className="text-green-400">Web Audio + Backend</span></span>
            </div>
            {/* Estado rápido */}
            <div className="flex gap-2 text-[9px]">
              <span className="flex items-center gap-1 text-cyan-400">
                <Activity size={10} /> {ultraLog.filter(l => l.type === 'sent').length} enviados
              </span>
              <span className="flex items-center gap-1 text-green-400">
                <Activity size={10} /> {ultraLog.filter(l => l.type === 'received').length} recibidos
              </span>
            </div>
          </div>
