...
# ============================================================
# GUARDIÁN DE ACCESO A ARCHIVOS — reglas de la casa de Sol
# ============================================================
# Sol puede leer/escribir SOLO dentro de sus propios hogares (los 3 repos
# + su carpeta de datos). Nunca fuera, nunca archivos de secretos.
# Regla de Harold (permanente): .env NO se toca. Nunca. Y los permisos
# siempre al mínimo necesario.

FORBIDDEN_PATTERNS = (".env", "secret", "credential", "token", ".pem", "id_rsa", "password")

def _safe_roots() -> list:
    """Directorios donde Sol SÍ puede tocar archivos."""
    roots = [p for p in ALLOWED_REPOS.values() if p.exists()]
    if SOL_HOME.exists():
        roots.append(SOL_HOME)
    return roots

def safe_resolve(ruta: str) -> Path:
    """Valida una ruta contra las reglas de la casa.
    Devuelve (Path_resuelta, None) si es segura, o (None, motivo) si no."""
    try:
        p = Path(ruta).expanduser()
        if not p.is_absolute():
            p = Path.cwd() / p
        p = p.resolve()
        # 1. ¿Dentro de una de las casas de Sol?
        if not any(p == r or r in p.parents for r in _safe_roots()):
            return None, f"❌ Fuera de mis hogares ({ruta}). Solo toco lo que vive en mis 3 repos y mi carpeta de datos — nada del sistema."
        # 2. ¿Archivo de secretos? (regla permanente de Harold: .env intocable)
        name = p.name.lower()
        if any(pat in name for pat in FORBIDDEN_PATTERNS):
            return None, f"❌ '{p.name}' es un archivo protegido. Los secretos (.env, tokens, llaves) no se leen ni se escriben — regla de la casa."
        return p, None
    except Exception as e:
        return None, f"❌ Ruta inválida: {e}"
...
def tool_read_file(path: str) -> str:
    try:
        p, motivo = safe_resolve(path)
        if p is None:
            return motivo
        if not p.exists():
            return f"❌ Archivo no existe: {path}"
        if p.is_dir():
            return f"❌ Es un directorio: {path}"
