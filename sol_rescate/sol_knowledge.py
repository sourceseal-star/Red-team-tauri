...
try:
    ...
    print(f"[sol_knowledge] No se pudo crear {KNOWLEDGE_DIR}: {_mkdir_err}")
...
def _github_token():
    """FIX: antes solo miraba GITHUB_ACCESS_TOKEN (nombre legado) — si Harold
    configuró el secreto con el nombre real documentado (GITHUB_TOKEN),
    este módulo lo veía vacío igual. Ahora acepta ambos, en vivo."""
    return os.environ.get("GITHUB_TOKEN") or os.environ.get("GITHUB_ACCESS_TOKEN", "")
GITHUB_REPOS = {
...
def build_knowledge_base(use_groq=True):
    ...
    # Guardar (defensivo: si el mkdir inicial falló, reintentamos aquí)
    try:
        KNOWLEDGE_DIR.mkdir(parents=True, exist_ok=True)
        ...
        print(f"[sol_knowledge] No se pudo guardar knowledge_full.json: {_write_err}")
    return knowledge
