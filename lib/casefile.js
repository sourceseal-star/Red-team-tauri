...
function probeApk() {
  ...
  try {
    ...
    // Sanitizar path — solo caracteres seguros en shell
    const safePath = apkPath.replace(/[;`$|&<>\n\r]/g, "");
    const out = execSync(`unzip -l "${safePath}" 2>/dev/null | head -30`, { encoding: 'utf8', timeout: 5000 });
    const dexMatch = out.match(/classes\d*\.dex/);
    if (dexMatch) {
      const dexData = execSync(`unzip -p "${safePath}" ${dexMatch[0]} 2>/dev/null`, { encoding: 'latin1', timeout: 10000, maxBuffer: 20 * 1024 * 1024 });
