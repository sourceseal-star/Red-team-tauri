...
const IOT_PORTS = [80, 443, 554, 3702, 8080, 8000, 8001, 8088, 37777];
...
function httpGet(ip, port, path, timeout = 2200) { return httpRaw(ip, port, path, {}, timeout); }

// ---- Digest handshake REAL (RFC 2617, qop=auth o sin qop) ----
...
async function httpAuthDigest(ip, port, path, user, pass, timeout = 3000) {
  const first = await httpRaw(ip, port, path, {});               // 1) sin auth
  if (first.status !== 401 || !/^Digest/i.test(first.www || '')) return first; // no es Digest
...
async function httpAuthAny(ip, port, path, user, pass) {
  // Basic primero; si 401+Digest, negocia Digest de verdad.
...
async function scan(ip) {
  ...
  const tcp = await Promise.all(IOT_PORTS.map(async p => {
  ...
  const httpPort = tcp.find(x => x.r.open && [80, 8080, 8000].includes(x.p));
  if (httpPort) {
    for (const c of CAM_PATHS) {
      ...
      if (r && r.status && r.status < 500) {
        ...
        if (type === 'unknown') type = 'camera'; break;
      }
    }
    if (type === 'unknown') {
...
async function scanMany(ips, conc = 6) {
  const out = []; let i = 0;
  const workers = Array.from({ length: conc }, async () => { while (i < ips.length) { const ip = ips[i++]; out.push(await scan(ip)); } });
  await Promise.all(workers); return out;
}

// ---- login-audit DEFENSIVO con Basic + Digest reales ----
const DEFAULTS = [['admin', 'admin'], ['admin', '12345']];   // tope: 2 pares, 1 puerto
async function loginAudit(ip, user, pass, port = 80) {
  ...
  if (!out.working) {
    for (const [du, dp] of DEFAULTS) {
      ...
      if (r.status === 200) { out.defaultCreds = { user: du, pass: dp || '(vacía)', scheme: r.scheme, cleartext: port !== 443 }; break; }
    }
  }
  const clear = out.tests.some(t => t.cleartext) || (out.defaultCreds && out.defaultCreds.cleartext);
  out.verdict = out.defaultCreds ? `CRÍTICO: acepta credenciales por defecto (${out.defaultCreds.scheme})`
    : clear ? 'ALTO: autenticación viaja en claro (HTTP)'
    : out.working ? `OK: auth sobre ${out.working.tls ? 'TLS' : 'HTTP'} vía ${out.working.scheme}`
    : 'sin acceso (credenciales incorrectas, o el dispositivo no expone ISAPI/web)';
  return out;
}

module.exports = { scan, scanMany, loginAudit };
