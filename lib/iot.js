...
// HTTP Digest Auth (RFC 2617) — MD5 es requerido por el protocolo, no es vulnerabilidad
...
function httpRaw(ip, port, path, headers, timeout = 2500) {
  return new Promise(res => {
    ...
    const req = proto.get({ host: ip, port, path, timeout, headers: Object.assign({ 'User-Agent': 'sealctl-audit/1.0' }, headers), rejectUnauthorized: true }, r => {
...
function sipOptions(ip, port = 5060, timeout = 2000) {
  return new Promise(res => {
    let s;
    try { s = dgram.createSocket('udp4'); } catch (e) { return res({ ok: false, banner: '' }); }
    let done = false;
    ...
    try { s.send(msg, port, ip, () => {}); } catch (e) { fin({ ok: false, banner: '' }); }
...
async function scan(ip, opts = {}) {
  ...
  return {
    ...
    return { ip, type: 'unknown', vendor: null, ports_open: [], services: [], evidence: [], summary: `error: ${e.message}` };
  }
}
...
async function scanMany(ips, conc = 12, opts = {}) {
  ...
  const workers = Array.from({ length: conc }, async () => {
    while (i < ips.length) {
      const ip = ips[i++];
      try { out.push(await scan(ip, opts)); }
      catch(e) { out.push({ ip, type: 'unknown', vendor: null, ports_open: [], services: [], evidence: [], summary: `error: ${e.message}` }); }
    }
  });
