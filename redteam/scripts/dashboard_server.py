...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        routes = {
            ...
            "/api/osint/shodan": self._api_shodan,
            "/api/exploits/list": self._api_exploits_list,
            "/api/scan/topology": self._api_scan_topology,
    ...
    def do_POST(self):
        ...
        if p == "/api/canary/svg/generate" or p == "/api/canary/generate":
            canary = _get_svg_canary()
            if not canary:
        ...
        if p == "/api/scan/topology":
            return self._json(self._post_scan_topology(body))
        if p == "/api/honeypot/start":
            return self._json(self._post_honeypot_start(body))
    ...
    def _api_shodan(self):
        """GET /api/osint/shodan?ip=8.8.8.8 — Lookup Shodan real."""
        parsed = urlparse(self.path)
        q = parse_qs(parsed.query)
        ip = q.get("ip", ["8.8.8.8"])[0]
        key = os.environ.get("SHODAN_API_KEY", "")
        if not key:
            return self._json({"error": "SHODAN_API_KEY no configurada", "ip": ip})
        try:
            import urllib.request
            url = f"https://api.shodan.io/shodan/host/{ip}?key={key}"
            req = urllib.request.Request(url, headers={"User-Agent": "SourceSeal/2.0"})
            with urllib.request.urlopen(req, timeout=10) as resp:
                import json as _j
                return self._json(_j.loads(resp.read().decode()))
        except Exception as e:
            return self._json({"error": str(e)[:200], "ip": ip}, 502)

    def _api_exploits_list(self):
        """GET /api/exploits/list — Lista exploits del directorio."""
        d = ROOT / "exploits"
        if not d.exists():
            return self._json({"results": []})
        return self._json({"results": [p.name for p in d.iterdir() if p.is_file()]})

    def _api_scan_topology(self):
        """GET /api/scan/topology — Descubrimiento de hosts con nmap."""
        import shlex
        subnet = _detect_local_network().get("cidr", "192.168.1.0/24")
        try:
        ...
        for line in out.splitlines():
            if "Nmap scan report for" in line:
                ...
                parts = line.split()
                current["mac"] = parts[2] if len(parts) > 2 else None
                current["vendor"] = " ".join(parts[3:]).strip("()") or "unknown"
        return self._json({"results": hosts, "hosts_up": len(hosts), "subnet": subnet})

    def _post_scan_topology(self, body):
        """POST /api/scan/topology — Descubrimiento de hosts con nmap."""
        import shlex
        subnet = body.get("subnet", "") or _detect_local_network().get("cidr", "192.168.1.0/24")
        try:
            cmd = f"nmap -sn -T3 {subnet}"
            out = subprocess.check_output(shlex.split(cmd),
                stderr=subprocess.DEVNULL, timeout=60).decode()
    ...
    def _post_honeypot_start(self, body):
        """POST /api/honeypot/start — Arranca honeypot Node.js real."""
        port = body.get("port", 8080)
        hp_script = ROOT.parent / "honeypot" / "start-honeypot.js"
        if not hp_script.exists():
            # Intentar el honeypot Python
            hp_script = ROOT / "scripts" / "honeypot.py"
            if not hp_script.exists():
                return {"error": "No se encontró script de honeypot"}
        try:
            proc = subprocess.Popen(
                ["node" if str(hp_script).endswith(".js") else "python3", str(hp_script), str(port)],
                stdout=open(str(LOGS_DIR / "honeypot.log"), "a"),
                stderr=subprocess.STDOUT)
            hp = _load_json(HONEYPOT_FILE, {})
            hp["active"] = True
            hp["pid"] = proc.pid
            hp["port"] = port
            hp["message"] = f"Honeypot arrancado en puerto {port}"
            _save_json(HONEYPOT_FILE, hp)
            _ws_broadcast({"type": "alert", "payload": f"Honeypot iniciado en puerto {port}"})
